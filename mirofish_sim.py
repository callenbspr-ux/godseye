...
AGENTS = ["Macro Bull", "Technical Bear", "Geopolitical Analyst", "Retail Sentiment", "Contrarian"]

# Agent weights for consensus calculation
AGENT_WEIGHTS = {
    "Macro Bull": 0.25,
    "Technical Bear": 0.20,
    "Geopolitical Analyst": 0.20,
    "Retail Sentiment": 0.15,
    "Contrarian": 0.20,
}
...
class MiroFishDebate:
    ...
    def get_output(self) -> Dict[str, Any]:
        ...
        return {
            "label": self.label,
            "consensus_bullish_pct": round(round_3_sent, 0),
            ...
            "sentiment_by_round": [round(round_1_sent, 0), round(round_2_sent, 0), round(round_3_sent, 0)],
...
def main():
    ...
    for instrument, metadata in INSTRUMENTS.items():
        print(f"[MIROFISH] Running 3-round debate for {instrument}...")

        debate = MiroFishDebate(instrument, market_data)

        # Run three rounds
        s1 = debate.run_round_1()
        print(f"  Round 1: {s1:.0f}% bullish")

        s2 = debate.run_round_2()
        print(f"  Round 2: {s2:.0f}% bullish")

        s3 = debate.run_round_3()
        print(f"  Round 3: {s3:.0f}% bullish")
